        ...
        <div class="modern-card p-4 mb-4" style="background: var(--theme-gradient);">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                <div class="text-white">
                    <h4 class="fw-bold mb-1"><i class="fas fa-edit me-2"></i> Input Nilai (Pribadi)</h4>
                    <p class="mb-0 opacity-75 fs-7">Input nilai rapor khusus untuk mapel yang Anda ajarkan.</p>
                </div>
        ...
        <div class="modal-content">
            ...
            <form action="<?= base_url(session()->get('role').'/akademik/simpan_nilai') ?>" method="post">
                <div class="modal-body p-4 bg-white">
                    <div class="row g-4">
                        <div class="col-md-12">
                            <label class="form-label fw-bold text-secondary text-uppercase fs-7">Pilih Tugas Mengajar</label>
                            <select name="tugas_mengajar" class="form-control modern-input" required>
                                <option value="">-- Kelas & Mapel --</option>
                ...
                <div class="modal-footer border-0 p-4 pt-0 bg-white">
                    ...
                    <button type="submit" class="btn text-white rounded-pill px-5 fw-bold shadow-sm border-0" style="background: var(--theme-gradient);"><i class="fas fa-save me-2"></i> Simpan</button>
                </div>
            </form>
        </div>
    </div>
...
<div class="modal fade" id="modalEdit<?= $row['id_nilai'] ?>" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header modal-header-custom">
                <h5 class="modal-title fw-bold text-white"><i class="fas fa-edit me-2"></i> Edit Nilai</h5>
                <button type="button" class="close text-white shadow-none" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            </div>
            <form action="<?= base_url(session()->get('role').'/akademik/update_nilai/' . $row['id_nilai']) ?>" method="post">
                ...
                <div class="modal-footer border-0 p-4 pt-0 bg-white">
                    <button type="button" class="btn btn-light rounded-pill px-4 fw-bold text-secondary shadow-sm border" data-dismiss="modal">Batal</button>
                    <button type="submit" class="btn text-white rounded-pill px-5 fw-bold shadow-sm border-0" style="background: var(--theme-gradient);"><i class="fas fa-save me-2"></i> Update</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header p-4" style="background: var(--theme-gradient);">
                <h5 class="modal-title fw-bold text-white"><i class="fas fa-file-excel me-2"></i> Import Nilai</h5>
                <button type="button" class="close text-white shadow-none" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            </div>
            <form action="<?= base_url(session()->get('role').'/akademik/import_nilai') ?>" method="post" enctype="multipart/form-data">
                <div class="modal-body p-4 bg-white text-center">
                    ...
                    <h6 class="fw-bold mb-3">Upload File Excel (.xlsx)</h6>
                ...
                <div class="modal-footer border-0 p-4 pt-0 bg-white d-flex justify-content-between">
                    ...
                    <button type="submit" class="btn text-white rounded-pill px-5 fw-bold shadow-sm border-0" style="background: var(--theme-gradient);"><i class="fas fa-upload me-2"></i> Proses</button>
                </div>
            </form>
        </div>
    </div>
