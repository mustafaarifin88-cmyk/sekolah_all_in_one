        ...
        <div class="modern-card p-4 mb-4" style="background: var(--theme-gradient);">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                <div class="text-white">
                    <h4 class="fw-bold mb-1"><i class="fas fa-newspaper me-2"></i> Publikasi Berita</h4>
                    <p class="mb-0 opacity-75 fs-7">Kelola artikel atau berita kegiatan yang Anda buat.</p>
                </div>
                ...
                <table class="table table-hover table-modern mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <img src="<?= base_url('uploads/berita/' . $row['thumbnail']) ?>" alt="Thumbnail" class="img-fluid rounded-3 shadow-sm" style="height: 70px; width: 100px; object-fit: cover;">
                            </td>
                            <td>
                            ...
                            <td class="text-center">
                                ...
                                <a href="<?= base_url('guru/informasi/hapus_berita/' . $row['id_berita']) ?>" onclick="return confirm('Hapus berita ini?')" class="btn btn-sm btn-light text-danger rounded-circle shadow-sm d-inline-flex align-items-center justify-content-center hover-lift" style="width:35px; height:35px;"><i class="fas fa-trash"></i></a>
                            </td>
                        </tr>
                        <?php endforeach; else: ?>
                        <tr><td colspan="5" class="text-center py-5 text-muted"><i class="fas fa-newspaper fs-1 mb-3 opacity-50 d-block"></i>Anda belum membuat berita apapun.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
...
<div class="modal fade" id="modalTambah" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content">
            ...
            <form action="<?= base_url('guru/informasi/simpan_berita') ?>" method="post" enctype="multipart/form-data">
...
<div class="modal fade" id="modalEdit<?= $row['id_berita'] ?>" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content">
            ...
            <form action="<?= base_url('guru/informasi/update_berita/' . $row['id_berita']) ?>" method="post" enctype="multipart/form-data">
                <div class="modal-body p-4 bg-light modal-body-scroll">
                    <div class="row g-4">
                        <div class="col-lg-8">
                            <div class="bg-white p-4 rounded-4 shadow-sm border border-light">
...
<script>
    function toggleTanggal(value, type, id = '') {
        ...
        if(value === 'schedule') { box.style.display = 'block'; } else { box.style.display = 'none'; }
    }
    ...
    window.addEventListener("load", function() {
        $('.summernote').summernote({ height: 350, toolbar: [ ['style', ['bold', 'italic', 'underline']], ['color', ['color']], ['para', ['ul', 'ol', 'paragraph']], ['insert', ['link', 'picture']], ['view', ['fullscreen']] ] });
    });
</script>
<?= $this->endSection() ?>