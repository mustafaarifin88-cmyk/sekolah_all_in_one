    ...
    <div class="col-lg-4">
        <div class="modern-card bg-white p-4 h-100">
            <div class="d-flex align-items-center mb-4">
                ...
                <h5 class="fw-bold m-0 text-dark">Tambah Sifat</h5>
            </div>
            <form action="<?= base_url('admin/administrasi/simpan_sifat_surat') ?>" method="post">
                <div class="mb-3">
                    <label class="form-label fw-bold text-secondary text-uppercase fs-7 tracking-wider">Sifat Surat</label>
                    <input type="text" class="form-control modern-input" name="sifat" placeholder="Contoh: Penting, Biasa" required>
                </div>
                <div class="mb-4">
                    <label class="form-label fw-bold text-secondary text-uppercase fs-7 tracking-wider">Keterangan</label>
                    <input type="text" class="form-control modern-input" name="keterangan" placeholder="Deskripsi singkat" required>
                </div>
                <button type="submit" class="btn text-white w-100 rounded-pill py-2 fw-bold shadow-sm hover-lift border-0" style="background: var(--theme-gradient);">
                    <i class="fas fa-save me-2"></i> Simpan Sifat
                </button>
            </form>
        </div>
    </div>
    <div class="col-lg-8">
        <div class="modern-card bg-white p-0 h-100 overflow-hidden">
            <div class="p-4 border-bottom border-light d-flex justify-content-between align-items-center bg-light">
                <h5 class="fw-bold m-0 text-dark"><i class="fas fa-tags me-2" style="color: var(--theme-color);"></i> Daftar Sifat Surat</h5>
            </div>
            <div class="table-responsive">
                <table class="table table-hover table-modern mb-0">
                    ...
                    <tbody>
                        <?php if(isset($sifat_surat) && count($sifat_surat) > 0): foreach($sifat_surat as $key => $row): ?>
                        <tr>
                            <td class="text-center fw-bold text-muted"><?= $key + 1 ?></td>
                            <td><span class="badge px-3 py-2 rounded-pill fs-6 text-white" style="background: var(--theme-color);"><?= $row['sifat'] ?></span></td>
                            <td class="fw-bold text-dark"><?= $row['keterangan'] ?></td>
                            <td class="text-center">
                                <button class="btn btn-sm btn-light text-primary rounded-circle shadow-sm me-1 hover-lift" data-toggle="modal" data-target="#modalEdit<?= $row['id_sifat_surat'] ?>" style="width:35px; height:35px;"><i class="fas fa-edit"></i></button>
                                <a href="<?= base_url('admin/administrasi/hapus_sifat_surat/' . $row['id_sifat_surat']) ?>" onclick="return confirm('Hapus sifat surat ini?')" class="btn btn-sm btn-light text-danger rounded-circle shadow-sm d-inline-flex align-items-center justify-content-center hover-lift" style="width:35px; height:35px;"><i class="fas fa-trash"></i></a>
                            </td>
                        </tr>
                        <?php endforeach; else: ?>
...
<?php if(isset($sifat_surat) && count($sifat_surat) > 0): foreach($sifat_surat as $row): ?>
<div class="modal fade" id="modalEdit<?= $row['id_sifat_surat'] ?>" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header modal-header-custom">
                <h5 class="modal-title fw-bold text-white"><i class="fas fa-edit me-2"></i> Edit Sifat Surat</h5>
            ...
            <form action="<?= base_url('admin/administrasi/update_sifat_surat/' . $row['id_sifat_surat']) ?>" method="post">
                <div class="modal-body p-4 bg-white">
                    <div class="mb-3">
                        <label class="form-label fw-bold text-secondary text-uppercase fs-7 tracking-wider">Sifat Surat</label>
                        <input type="text" class="form-control modern-input" name="sifat" value="<?= $row['sifat'] ?>" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-bold text-secondary text-uppercase fs-7 tracking-wider">Keterangan</label>
