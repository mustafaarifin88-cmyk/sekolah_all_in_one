    ...
    <div class="col-lg-4">
        <div class="modern-card bg-white p-4 h-100">
            <div class="d-flex align-items-center mb-4">
                ...
                <h5 class="fw-bold m-0 text-dark">Input Beban</h5>
            </div>
            <form action="<?= base_url('admin/kepegawaian/simpan_beban_kerja') ?>" method="post">
                <div class="mb-3">
                    <label class="form-label fw-bold text-secondary text-uppercase fs-7 tracking-wider">Pilih Guru/Tendik</label>
                ...
                <div class="mb-4">
                    <label class="form-label fw-bold text-secondary text-uppercase fs-7 tracking-wider">Total Jam Kerja / Minggu</label>
                    <div class="input-group">
                        <input type="number" class="form-control modern-input rounded-end-0" name="jumlah_jam_kerja" placeholder="Contoh: 24" required>
                        <span class="input-group-text bg-light border-0 text-dark px-3 rounded-end-3 fw-bold">Jam</span>
                    </div>
                </div>
                <button type="submit" class="btn text-white w-100 rounded-pill py-2 fw-bold shadow-sm hover-lift border-0" style="background: var(--theme-gradient);">
                    <i class="fas fa-save me-2"></i> Simpan Beban
                </button>
            </form>
        </div>
    </div>
    <div class="col-lg-8">
        <div class="modern-card bg-white p-0 h-100 overflow-hidden">
            <div class="p-4 border-bottom border-light d-flex justify-content-between align-items-center bg-light">
                <h5 class="fw-bold m-0 text-dark"><i class="fas fa-tasks me-2" style="color: var(--theme-color);"></i> Distribusi Beban Kerja</h5>
            </div>
                ...
                <table class="table table-hover table-modern mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center"><span class="badge px-3 py-2 rounded-pill fs-6 text-white" style="background: var(--theme-color);"><?= $row['jumlah_jam_kerja'] ?> Jam / Minggu</span></td>
                            <td class="text-center">
                                <button class="btn btn-sm btn-light text-primary rounded-circle shadow-sm me-1 hover-lift" data-toggle="modal" data-target="#modalEdit<?= $row['id_beban'] ?>" style="width:35px; height:35px;"><i class="fas fa-edit"></i></button>
                                <a href="<?= base_url('admin/kepegawaian/hapus_beban_kerja/' . $row['id_beban']) ?>" onclick="return confirm('Hapus data ini?')" class="btn btn-sm btn-light text-danger rounded-circle shadow-sm d-inline-flex align-items-center justify-content-center hover-lift" style="width:35px; height:35px;"><i class="fas fa-trash"></i></a>
                            </td>
                        </tr>
                        <?php endforeach; else: ?>
                        <tr><td colspan="4" class="text-center py-4 text-muted">Belum ada beban kerja yang diinput.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
...
<?php if(isset($beban) && count($beban) > 0): foreach($beban as $row): ?>
<div class="modal fade" id="modalEdit<?= $row['id_beban'] ?>" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header modal-header-custom">
                <h5 class="modal-title fw-bold text-white"><i class="fas fa-edit me-2"></i> Edit Beban Kerja</h5>
            ...
            <form action="<?= base_url('admin/kepegawaian/update_beban_kerja/' . $row['id_beban']) ?>" method="post">
                <div class="modal-body p-4 bg-white">
                    ...
                    <div class="mb-4">
                        <label class="form-label fw-bold text-secondary text-uppercase fs-7 tracking-wider">Total Jam Kerja</label>
                        <div class="input-group">
                            <input type="number" class="form-control modern-input rounded-end-0" name="jumlah_jam_kerja" value="<?= $row['jumlah_jam_kerja'] ?>" required>
                            <span class="input-group-text bg-light border-0 text-dark px-3 rounded-end-3 fw-bold">Jam</span>
                        </div>
                    </div>
                </div>
