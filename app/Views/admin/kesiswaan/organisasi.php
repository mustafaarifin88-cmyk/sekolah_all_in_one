    ...
    <div class="col-lg-4">
        <div class="modern-card bg-white p-4 h-100">
            <div class="d-flex align-items-center mb-4">
                ...
                <h5 class="fw-bold m-0 text-dark">Data Organisasi</h5>
            </div>
            <form action="<?= base_url('admin/kesiswaan/simpan_organisasi') ?>" method="post">
                <div class="mb-4">
                    <label class="form-label fw-bold text-secondary text-uppercase fs-7 tracking-wider">Nama Organisasi</label>
                    <input type="text" class="form-control modern-input" name="nama_organisasi" placeholder="Contoh: OSIS, MPK" required>
                </div>
    ...
    <div class="col-lg-8">
        <div class="modern-card bg-white p-0 h-100 overflow-hidden">
            <div class="p-4 border-bottom border-light d-flex justify-content-between align-items-center bg-light">
                <h5 class="fw-bold m-0 text-dark"><i class="fas fa-list me-2" style="color: #fda085;"></i> Master Data Organisasi Siswa</h5>
            </div>
            <div class="table-responsive">
                <table class="table table-hover table-modern mb-0">
                    ...
                    <tbody>
                        <?php if(isset($organisasi) && count($organisasi) > 0): foreach($organisasi as $key => $row): ?>
                        <tr>
                            <td class="text-center fw-bold text-muted"><?= $key + 1 ?></td>
                            <td class="fw-bold text-dark fs-6"><?= $row['nama_organisasi'] ?></td>
                            <td class="text-center">
                                <button class="btn btn-sm btn-light text-primary rounded-circle shadow-sm me-1 hover-lift" data-toggle="modal" data-target="#modalEdit<?= $row['id_organisasi'] ?>" style="width:35px; height:35px;"><i class="fas fa-edit"></i></button>
                                <a href="<?= base_url('admin/kesiswaan/hapus_organisasi/' . $row['id_organisasi']) ?>" onclick="return confirm('Hapus organisasi ini?')" class="btn btn-sm btn-light text-danger rounded-circle shadow-sm d-inline-flex align-items-center justify-content-center hover-lift" style="width:35px; height:35px;"><i class="fas fa-trash"></i></a>
                            </td>
                        </tr>
                        <?php endforeach; else: ?>
                        <tr><td colspan="3" class="text-center py-4 text-muted">Belum ada data organisasi.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
...
<?php if(isset($organisasi) && count($organisasi) > 0): foreach($organisasi as $row): ?>
<div class="modal fade" id="modalEdit<?= $row['id_organisasi'] ?>" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header modal-header-custom">
                <h5 class="modal-title fw-bold text-dark"><i class="fas fa-edit me-2"></i> Edit Organisasi</h5>
            ...
            <form action="<?= base_url('admin/kesiswaan/update_organisasi/' . $row['id_organisasi']) ?>" method="post">
                <div class="modal-body p-4 bg-white">
                    <div class="mb-4">
                        <label class="form-label fw-bold text-secondary text-uppercase fs-7 tracking-wider">Nama Organisasi</label>
                        <input type="text" class="form-control modern-input" name="nama_organisasi" value="<?= $row['nama_organisasi'] ?>" required>
                    </div>
                </div>
